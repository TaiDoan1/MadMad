...
// 4b. PUT /api/orders/:id/customer-cancel - Khách hàng tự hủy đơn (Public, không cần Admin)
// Bảo mật: bắt buộc khớp đúng SĐT của đơn hàng + chỉ cho hủy trong 5 phút đầu + trạng thái còn hủy được
router.put("/:id/customer-cancel", async (req, res, next) => {
  try {
    const orderId = Number(req.params.id);
    const { customerPhone } = req.body;

    if (!customerPhone) {
      return res.status(400).json({ message: "Thiếu số điện thoại xác thực!" });
    }
    ...
    const normalizedInput = String(customerPhone).replace(/\s+/g, "");
    const normalizedOrderPhone = String(existingOrder.customerPhone || "").replace(/\s+/g, "");
    if (!normalizedOrderPhone || normalizedInput !== normalizedOrderPhone) {
      return res.status(403).json({ message: "Số điện thoại không khớp với đơn hàng!" });
    }

    if (isInactiveOrderStatus(existingOrder.status)) {
      return res.status(400).json({ message: "Đơn hàng đã được hủy hoặc hoàn tất trước đó!" });
    }

    const FIVE_MINUTES_MS = 5 * 60 * 1000;
    const orderAgeMs = Date.now() - new Date(existingOrder.createdAt).getTime();
    if (orderAgeMs > FIVE_MINUTES_MS) {
      return res.status(400).json({ message: "Đã quá thời gian cho phép tự hủy đơn (5 phút)!" });
    }
    ...
    const updatedOrder = await prisma.$transaction(async (tx) => {
      await restoreOrderItemsIfDeducted(
        ...
        "Hoàn kho do khách tự hủy đơn hàng",
        "ORDER_CANCEL",
      );
