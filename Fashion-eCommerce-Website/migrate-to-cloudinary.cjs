...
// 1. Tải cấu hình biến môi trường từ backend/.env
const backendEnvPath = path.join(__dirname, "backend", ".env");
if (fs.existsSync(backendEnvPath)) {
  require("dotenv").config({ path: backendEnvPath });
  console.log("✅ Đã nạp file cấu hình backend/.env");
} else {
  console.error("❌ Không tìm thấy file cấu hình backend/.env. Vui lòng kiểm tra lại đường dẫn.");
  process.exit(1);
}
